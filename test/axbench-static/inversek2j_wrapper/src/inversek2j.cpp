...
int main(int argc, const char* argv[])
{
	...
	for (int i=0; i<n*2*2; i+=2*2) {
		float __attribute((annotate("scalar(range('_MIN_THETA1_RANGE', '_MAX_THETA1_RANGE') error(1e-8) disabled)"))) theta1, __attribute((annotate("scalar(range('_MIN_THETA2_RANGE', '_MAX_THETA2_RANGE') error(1e-8) disabled)"))) theta2;
...
extern "C"
void newmain(float* t1t2xy_, int n) {

	srand (time(NULL));
	float* __attribute((annotate("target('t1t2xy') scalar()"))) t1t2xy = t1t2xy_;
	AxBenchTimer timer;
	int curr_index1 = 0;
	for(int i = 0 ; i < n * 2 * 2 ; i += 2 * 2)
	{
